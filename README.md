...
#### 1. 处理资源包伪装头（`ver00d0.arc`）
这些包通常只有一个伪装头，没有拼接。新建一个 `FixArc.py`，复制以下代码并运行：
```python
import os
...
#### 2. 处理巨型资源包（`ver00dxxx.arc`）
这些 1GB 的大包是“连体婴儿”，用fixarc修改完之后需要切分。新建一个 `SplitBundles.py`，复制以下代码并运行：
```python
import os
